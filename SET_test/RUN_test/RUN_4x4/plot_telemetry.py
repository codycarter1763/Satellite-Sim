...
# ------------------------------------------------------------
# 15. Orbital-plane coordinate system
# ------------------------------------------------------------
...
# Eccentricity vector
#
# Points toward periapsis and lies in the orbital plane
# ------------------------------------------------------------

e_vec = (
    ((v0_mag**2 - MU_EARTH / r0_mag) * r0)
    - (np.dot(r0, v0) * v0)
) / MU_EARTH
...
# Y-axis completes right-handed orbital-plane coordinate system
y_hat = np.cross(h_hat, x_hat)
y_hat = y_hat / np.linalg.norm(y_hat)

# ------------------------------------------------------------
# Transform ECI position into orbital-plane coordinates
# ------------------------------------------------------------

r_vectors = df_valid[["px", "py", "pz"]].to_numpy()

orbital_x = r_vectors @ x_hat / 1000.0
orbital_y = r_vectors @ y_hat / 1000.0
...
plt.xlabel("Orbital-Plane X (km)")
plt.ylabel("Orbital-Plane Y (km)")
plt.title("Spacecraft Orbital Path")

# Critical: prevent the ellipse from being visually distorted
plt.axis("equal")
